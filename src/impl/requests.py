...
class Client(object):
    ...
    def mobileSession(self):
        if self._sk is None:
            ret=self.call_GET(addSign=True, method="auth.getMobileSession", username=self._username,
    ...
    def call_POST(self, **paramsMap):
        ...
        paramsMap.update({'api_sig':self._get_sign(paramsMap)}
                         ) #NB: api_sig must be calculated after all params have been supplied
...
class UserRequest(Request):
    ...
    def getShouts(self, limit=50, autocorrect=0, page=None):
        ...
        ret=self._client.call_GET(method="user.getShouts", user=self._name, autocorrect=autocorrect, limit=limit,
                                  page=page)
...
class ArtistRequest(Request):
    ...
    def getShouts(self, limit=50, autocorrect=0, page=None):
        ...
        ret=self._client.call_GET(method="artist.getShouts", artist=self._name, autocorrect=autocorrect, limit=limit,
                                  page=page)
        return _getShoutsHandler(ret)
    ...
    def addTags(self, tags):
        ...
        ret=self._client.call_POST(method='artist.addTags', artist=self._name,
                                   tags=','.join(tag.getName() for tag in tags))
        return _statusHandler(ret)
    ...
    def share(self, recipients, message=None, public=0):
        ...
        ret=self._client.call_POST(artist=self._name, method="artist.share", message=message, public=public,
    ...
    def _getInfo(self, lang='en', autocorrect=0, username=None):
        ...
        return self._client.call_GET(method="artist.getInfo", artist=self._name, lang=lang, autocorrect=autocorrect,
                                     username=username)
...
class EventRequest(Request):
    ...
    def share(self, recipients, message=None, public=0):
        ...
        ret=self._client.call_POST(event=self._id, method="event.share", message=message, public=public,
                                   recipient=','.join(recipients))
        return _statusHandler(ret)
...
class TrackRequest(Request):
    ...
    def _getInfo(self, autocorrect=0, username=None):
        ...
        return self._client.call_GET(method="track.getInfo", track=self._name, artist=self._artist.getName(),
    ...
    def getShouts(self, limit=50, autocorrect=0, page=None):
        ...
        ret=self._client.call_GET(method="track.getShouts", track=self._name, artist=self._artist.getName(),
                                  autocorrect=autocorrect, limit=limit, page=page)
        return _getShoutsHandler(ret)
    ...
    def share(self, recipients, message=None, public=0):
        ...
        ret=self._client.call_POST(track=self._name, artist=self._artist.getName(), method="track.share",
                                   message=message, public=public, recipient=','.join(recipients))
        return _statusHandler(ret)
    ...
    def addTags(self, tags):
        ...
        ret=self._client.call_POST(method='track.addTags', track=self._name, artist=self._artist.getName(),
                                   tags=','.join(tag.getName() for tag in tags))
        return _statusHandler(ret)

    def removeTag(self, tag):
        ret=self._client.call_POST(method='track.removeTag', track=self._name, artist=self._artist.getName(),
                                   tag=tag.getName())
        return _statusHandler(ret)
    ...
    def getTags(self, autocorrect=0):
        ...
        ret=self._client.call_POST(method='track.getTags', track=self._name, artist=self._artist.getName(),
    ...
    def getTopTags(self, autocorrect=0):
        ...
        ret=self._client.call_GET(method="track.getTopTags", autocorrect=autocorrect, artist=self._artist.getName(),
                                  track=self._name)
        result=[]
        for tag in xmlutils.extract_elems(ret, ".//toptags/tag"):
            name=xmlutils.extract_subelem(tag, ".//name").text
    ...
    def getTopFans(self, autocorrect=0):
        ...
        ret=self._client.call_GET(method="track.getTopFans", autocorrect=autocorrect, artist=self._artist.getName(),
                                  track=self._name)
        result=[]
        for user in xmlutils.extract_elems(ret, ".//topfans/user"):
            name=xmlutils.extract_subelem(user, ".//name").text
    ...
    def getBuylinks(self, autocorrect=0, country='united kingdom'):
        ...
        ret=self._client.call_GET(method="track.getBuylinks", country=country, track=self._name,
                                  artist=self._artist.getName(), autocorrect=autocorrect)
...
class AlbumRequest(Request):
    ...
    def _getInfo(self, autocorrect=0, username=None, lang='en'):
        ...
        return self._client.call_GET(method="album.getInfo", album=self._name, artist=self._artist.getName(),
                                     autocorrect=autocorrect, username=username, lang=lang)
    ...
    def addTags(self, tags):
        ...
        ret=self._client.call_POST(method='album.addTags', album=self._name, artist=self._artist.getName(),
                                   tags=','.join(tag.getName() for tag in tags))
        return _statusHandler(ret)

    def removeTag(self, tag):
        ret=self._client.call_POST(method='album.removeTag', album=self._name, artist=self._artist.getName(),
                                   tag=tag.getName())
        return _statusHandler(ret)
    ...
    def getTags(self, autocorrect=0):
        ...
        ret=self._client.call_POST(method='album.getTags', album=self._name, artist=self._artist.getName(),
                                   autocorrect=autocorrect)
        return [TagRequest(client=self._client, name=xmlutils.extract_subelem(tag, "name").text,
                           url=xmlutils.extract_subelem(tag, "url").text) for tag in
                xmlutils.extract_elems(ret, ".//tags/tag")]
    ...
    def share(self, recipients, message=None, public=0):
        ...
        ret=self._client.call_POST(album=self._name, artist=self._artist.getName(), method="album.share",
                                   message=message, public=public, recipient=','.join(recipients))
        return _statusHandler(ret)
    ...
    def getShouts(self, limit=50, autocorrect=0, page=None):
        ...
        ret=self._client.call_GET(method="album.getShouts", album=self._name, artist=self._artist.getName(),
                                  autocorrect=autocorrect, limit=limit, page=page)
        return _getShoutsHandler(ret)
    ...
    def getBuylinks(self, autocorrect=0, country='united kingdom'):
        ...
        ret=self._client.call_GET(method="album.getBuylinks", country=country, album=self._name,
                                  artist=self._artist.getName(), autocorrect=autocorrect)
...
class LibraryRequest(Request):
    ...
    def addTrack(self, track):
        ret = self._client.call_POST(method="library.addTrack", artist=track.getArtist().getName(),
                                     track=track.getName())
        return _statusHandler(ret)
